...
export default function MainLayout() {
  ...
  return <div className="flex min-h-screen flex-col">
    <header className="border-b border-stone-200 bg-white"><div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-4 px-5 py-5">
      ...
      <nav className="flex flex-wrap items-center gap-4 text-sm text-emerald-800"><Link to="/services">Dịch vụ</Link>{user?.role === 'admin' && <><Link to="/admin/categories">Danh mục</Link><Link to="/admin/services">Quản lý dịch vụ</Link></>}</nav>
