...
export default function MainLayout() {
  ...
  return <div className="flex min-h-screen flex-col">
    <header className="border-b border-stone-200 bg-white"><div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-4 px-5 py-5">
      ...
      <nav className="flex flex-wrap items-center gap-4 text-sm text-emerald-800">{user?.role !== 'admin' && <><Link to="/services">Dịch vụ</Link><Link to="/promotions">Khuyến mãi</Link></>}{user?.role === 'admin' && <><Link to="/admin/categories">Danh mục</Link><Link to="/admin/services">Quản lý dịch vụ</Link><Link to="/admin/employees">Nhân viên</Link><Link to="/admin/appointments">Lịch hẹn</Link><Link to="/admin/promotions">Khuyến mãi</Link></>}{user?.role === 'user' && <Link to="/appointments">Lịch hẹn của tôi</Link>}{user && <Link to={user.role === 'admin' ? '/admin/account' : '/account'}>Tài khoản</Link>}</nav>
