...
export default function EmployeeDetailPage() {
  ...
  return <section className="space-y-6">
    ...
    <section className="rounded-2xl border border-stone-200 bg-white p-6"><h2 className="text-xl font-semibold">Phân công dịch vụ</h2>
      ...
      <ul className="divide-y divide-stone-200">{assignments.map(item => <li key={item.service_id} className="flex flex-wrap items-center justify-between gap-3 py-3"><div><strong>{item.service_name}</strong><p className="text-sm text-stone-600">{item.status === 'active' ? 'Đang phụ trách' : 'Đã ngừng phụ trách'}</p></div><button className="text-emerald-800 underline" disabled={busy} onClick={() => perform(() => updateAssignment(id, item.service_id, item.status === 'active' ? 'inactive' : 'active'), 'Đã cập nhật phân công.')}>{item.status === 'active' ? 'Ngừng phụ trách' : 'Kích hoạt lại'}</button></li>)}</ul>{!assignments.length && <p className="text-stone-500">Chưa có phân công.</p>}
    </section>
